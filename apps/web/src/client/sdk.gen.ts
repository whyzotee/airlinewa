...
import type { AirportGetAirportsData, AirportGetAirportsResponse, UserUsersData, AuthLoginData, AuthLoginError, FlightSearchFlightData, FlightSearchFlightError, PaymentPaymentsData, PaymentPaymentsError, PaymentCheckoutData, PaymentCheckoutError, UtilsTestEmailData, UtilsTestEmailError, UtilsHealthCheckData, UtilsHealthCheckResponse } from './types.gen';
...
export const flightSearchFlight = <ThrowOnError extends boolean = false>(options: Options<FlightSearchFlightData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, FlightSearchFlightError, ThrowOnError>({
        url: '/api/flight/',
        ...options
    });
};
